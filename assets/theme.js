...
Slate.simplifyVariantLabels = function (product, container) {
  ...
  if (product.options.length === 1 && product.options[0] !== 'Title') {
    $('.selector-wrapper:eq(0)', container).prepend('<label for="ProductSelect-option-0">'+ product.options[0] +'</label>');
  }
  ...
  theme.sliders = function (slider) {
    ...
    this.sliderArgs = {
      ...
      start: function(slider) {
        moveControlNav(slider.animatingTo);
        $(window).on('resize.slideshow--' + slider.data('section-id'), function() { moveControlNav(slider.animatingTo) });
      }
    }
    ...
    function moveControlNav(slideNumber) {
      ...
      if (theme.variables.bpSmall) {
        var slideCTA = $(slider).find('li[data-flexslider-index=\'' + slideNumber + '\']').find('.slide-cta');
        if (slideCTA.length == 0) {
          var slideCTA = $(slider).find('li[data-flexslider-index=\'' + slideNumber + '\']').find('.slide-cta-placeholder');
        }
        if (slideCTA.length != 0) {
...
theme.Product = (function() {
  function Product(container) {
    ...
    this.selectors = {
      ...
      $productImageWrappers: $('.product-single__featured-image-wrapper', this.$container),
      $productSinglePhotos: $('.product-single__photos', this.$container),
      $productThumbs: $('#ProductThumbs').find('.product-single__thumbnail', this.$container),
    ...
    this.productSingleObject = JSON.parse(document.getElementById('ProductJson-' + this.sectionId).innerHTML);
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
        ...
        enquire.register(theme.variables.mediaQuerySmall, {
          match: function() {
            if (zoomType == 'zoom-in') {
              ...
              self.selectors.$productSinglePhotos.find('img').removeClass('lightbox');
            }
          ...
          unmatch: function() {
            if (zoomType == 'zoom-in') {
              ...
              // reinit product zoom
              this.selectors.$productSinglePhotos.each(function( index ) {
              ...
              self.selectors.$productSinglePhotos.find('img').addClass('lightbox');
            }
    ...
    productImageGallery: function() {
      if (!this.selectors.$productImageGallery.length) {
        return;
      };
      ...
      this.selectors.$productSinglePhotos.bind('click', function() {
        ...
        self.selectors.$productImageGallery.filter('[data-image-id="' + imageId + '"]').trigger('click');
      });
    },
    ...
    productPage: function (options) {
      ...
      if (variant) {
        ...
        this.selectors.$productPrice.html(Shopify.formatMoney(variant.price, theme.moneyFormat));

        // Update and show the product's compare price if necessary
        if (variant.compare_at_price > variant.price) {
          this.selectors.$comparePrice.html(Shopify.formatMoney(variant.compare_at_price, theme.moneyFormat)).removeClass('hide');
          this.selectors.$productPrice.addClass('product-single__price--on-sale');
          this.selectors.$comparePriceA11y.attr('aria-hidden', 'false');
          this.selectors.$priceA11y.attr('aria-hidden', 'false');
        } else {
          this.selectors.$comparePrice.addClass('hide');
          this.selectors.$productPrice.removeClass('product-single__price--on-sale');
        ...
        if ($link.length) {
          $link.attr('href', this.updateUrlParameter($link.attr('href'), 'variant', variant.id));
        }
    ...
    _productImageZoom: function() {
      ...
      if (!theme.variables.bpSmall && zoomType == 'zoom-in'){
        if (!this.selectors.$productImageWrappers.length || this.selectors.$html.hasClass('supports-touch')) {
          return;
        };
...
theme.SlideshowSection.prototype = _.assignIn({}, theme.SlideshowSection.prototype, {
  ...
  onBlockSelect: function(evt) {
    ...
    var $slide = $('#slide--' + evt.originalEvent.detail.blockId + ':not(.clone)');
...
theme.Maps = (function() {
  ...
  window.gm_authFailure = function() {
    if (Shopify.designMode) {
      ...
      $(selectors.mapOverlay).after('<div class="' + classes.errorMsg + '">' + theme.mapStrings.authError + '</div>');
    }
  }
